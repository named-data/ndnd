...
func (t *Thread) processIncomingInterest(packet *defn.Pkt) {
	...
	if interest.HopLimit() != nil {
		core.Log.Trace(t, "HopLimit check", "name", packet.Name, "hoplimit", *interest.HopLimit())
	...
	if exists := t.deadNonceList.Find(interest.Name(), *interest.Nonce()); exists {
		core.Log.Debug(t, "Interest is looping (DNL)", "name", packet.Name, "nonce", *interest.Nonce())
		return
	}
